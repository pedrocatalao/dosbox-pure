...
Bitu GFX_SetSize(Bitu width, Bitu height, Bitu flags, double scalex, double scaley, GFX_CallBack_t cb)
{
	...
	//DBP: DOS ex Machina is told whether this mode is a text mode (dosbox_pure_dxm.h)
	DXM_TextMode(vga.mode == M_TEXT || vga.mode == M_HERC_TEXT || vga.mode == M_TANDY_TEXT);
...
static void init_dosbox(bool forcemenu = false, bool reinit = false, const std::string* dbconf = NULL)
{
	...
	PROGRAMS_MakeFile("CATALOG.COM", DBP_DXMCatalogProgram);
	DXM_RegisterFiles();
